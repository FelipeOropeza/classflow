...
<template>
  <Head title="Gestao de Responsaveis" />
    ...
    <div class="max-w-6xl mx-auto space-y-8 py-6 animate-in fade-in duration-700">
      ...
      <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 px-1">
        <div class="space-y-1">
          ...
          <p class="text-slate-400 text-sm font-medium italic">Cadastre e gerencie os dados de acesso para os pais e responsaveis.</p>
        </div>
        ...
        >
          <UserPlus :size="16" /> Novo Responsavel
        </button>
      </div>
            ...
            <table class="w-full text-left border-collapse">
               <thead>
                  <tr class="bg-slate-50/50">
                     ...
                     <th class="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Nivel de Acesso</th>
                  ...
                  <tr v-if="users.length === 0">
                    <td colspan="4" class="px-8 py-16 text-center">
                      <div class="flex flex-col items-center gap-3">
                        ...
                        <p class="text-sm font-bold text-slate-400">Nenhum responsavel cadastrado ainda.</p>
                      </div>
                    </td>
                  </tr>
               </tbody>
      ...
      <div v-if="showCreateModal" class="fixed inset-0 z-50 flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-300">
         <div class="w-full max-w-xl bg-white rounded-[32px] p-10 shadow-2xl space-y-8 animate-in zoom-in-95 duration-300">
            <div class="flex items-center justify-between">
               <h3 class="text-xl font-bold text-slate-900 tracking-tight">Novo Acesso</h3>
               <button @click="showCreateModal = false; form.reset(); form.clearErrors();" class="text-slate-400 hover:text-slate-600 transition-colors text-sm font-bold">Fechar</button>
            </div>
            ...
            <form @submit.prevent="submit" class="space-y-5">
               ...
               <div class="space-y-2">
                  ...
                  <div class="relative group">
                     <input
                       ...
                       :class="['w-full bg-slate-50 rounded-2xl px-5 py-3.5 pl-10 text-xs font-bold focus:ring-4 focus:ring-indigo-500/10 transition-all outline-none border', form.errors.name ? 'border-red-400 bg-red-50/30' : 'border-slate-200 focus:border-indigo-500']"
                     >
               ...
               <div class="space-y-2">
                  ...
                  <div class="relative group">
                     <input
                       ...
                       :class="['w-full bg-slate-50 rounded-2xl px-5 py-3.5 pl-10 text-xs font-bold focus:ring-4 focus:ring-indigo-500/10 transition-all outline-none border', form.errors.email ? 'border-red-400 bg-red-50/30' : 'border-slate-200 focus:border-indigo-500']"
                     >
                  ...
                  <div class="space-y-2">
                     ...
                     <div class="relative group">
                        <input
                          ...
                          :class="['w-full bg-slate-50 rounded-2xl px-5 py-3.5 pl-10 text-xs font-bold focus:ring-4 focus:ring-indigo-500/10 transition-all outline-none border', form.errors.password ? 'border-red-400 bg-red-50/30' : 'border-slate-200 focus:border-indigo-500']"
                        >
                  ...
                  <div class="space-y-2">
                     ...
                     <div class="relative group">
                        <input
                          ...
                          class="w-full bg-slate-50 border-slate-200 rounded-2xl px-5 py-3.5 pl-10 text-xs font-bold focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all outline-none border"
                        >
                        <Key class="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-indigo-500 transition-colors" :size="16" />
                     </div>
                  </div>
               ...
               <div class="pt-2">
                  <button type="submit" :disabled="form.processing" class="w-full bg-indigo-600 hover:bg-slate-900 text-white font-bold py-4 rounded-2xl transition-all shadow-xl shadow-indigo-100 active:scale-95 flex items-center justify-center gap-2 disabled:opacity-50">
